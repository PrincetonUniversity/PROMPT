...
Inline void sq_produceDouble(SW_Queue q, double value, sq_callback cb, sq_cbData cbd) {
  uint64_t pDataRaw = q->p_data;
  uint32_t pInx = sq_pInx(q);
  uint64_t *pData = (uint64_t *) (size_t) (uint32_t) q->p_data;
  uint64_t *ptr = pInx + pData;
  uint64_t valueInt = (uint64_t)doubleToInt(value);
  sq_write(ptr, valueInt);
...
#define sq_produceDouble(Q, V) sq_produceDouble(Q, V, (sq_callback) sq_flushQueue, Q)
...
Inline double sq_consumeDouble(SW_Queue q, sq_callback cb, sq_cbData cbd)
{
#ifdef INSTRUMENT
  q->total_consumes++;
#endif
  ...
  q->c_inx++;
  q->c_inx &= QMASK;
  double valDouble = intToDouble((int64_t)val);
  return valDouble;
}

#define sq_consumeDouble(Q) sq_consumeDouble(Q, (sq_callback) sq_reverseFlush, Q)

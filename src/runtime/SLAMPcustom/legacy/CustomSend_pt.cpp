...
static void produce_32_64(uint32_t x, uint64_t y) ATTRIBUTE(noinline){
#ifdef MM_STREAM
  // _mm_stream_si128((__m128i *)(dq_data + dq_index), _mm_set_epi32(0, 0, y, x));
  _mm_stream_si32((int *) &dq_data[dq_index], x);
...
//     FINISHED
// };
enum PointsToModAction : uint32_t {
  INIT = 0,
  // LOAD,
  // STORE,
  ALLOC,
...
void SLAMP_enter_fcn(uint32_t id){
  produce_32_32(FUNC_ENTRY, id);
}

void SLAMP_exit_fcn(uint32_t id){
  produce_32_32(FUNC_EXIT, id);
}
...
static void SLAMP_free_hook(void *ptr, const void *caller){
  TURN_OFF_HOOKS
  free(ptr);
  produce_32_64(FREE, (uint64_t)ptr);
  TURN_ON_HOOKS
}
